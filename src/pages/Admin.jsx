...
export default function Admin() {
  ...
  const handleStart = () => {
    // Start fresh but do NOT call resetGame here
    setStep(0);
  ...
  return (
    <div className="container">
      ...
      <div style={{ marginBottom: 20 }}>
        <button onClick={handleReset} style={{ backgroundColor: '#f44336', color: 'white', padding: '10px 20px' }}>
          Reset Game
        </button>
      </div>
      ...
      {(step === -1 || step === "done" || !questions[questionIndex]) && (
        <>
          <button onClick={handleStart} style={{ backgroundColor: '#4caf50', color: 'white', padding: '10px 20px' }}>
            Start Game
          </button>
          {step === "done" && (
            <>
