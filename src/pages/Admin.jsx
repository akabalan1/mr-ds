...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    if (active && timer > 0) {
      ...
      console.log("Timer ended, calculating tally for Q", questionIndex);
      calculateTally();
  ...
  if (step === "done" || !questions[questionIndex]) {
    return (
      <div className="container">
        <h2>Game Over!</h2>
        ...
        <button onClick={handleReset} style={{ marginTop: 20, backgroundColor: '#f44336', color: 'white', padding: '10px 20px' }}>Reset Game</button>
      </div>
    );
  }
  ...
  return (
    <div className="container">
      ...
      <button onClick={() => {
        console.log("Admin clicked Show Results. Question index:", questionIndex);
        calculateTally();
        playSound(resultSound);
        setShowChart(true);
        setActive(false);
      ...
      <button onClick={handleReset} style={{ marginTop: 40, backgroundColor: '#f44336', color: 'white', padding: '10px 20px' }}>Reset Game</button>
    </div>
  );
}
